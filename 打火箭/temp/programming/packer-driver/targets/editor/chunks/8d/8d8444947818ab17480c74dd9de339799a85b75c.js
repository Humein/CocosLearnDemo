          ...
          this.schedule(() => {
            ...
            for (let i = 0; i < numberOfUFOs; i++) {
              if (i === 0) {
                const targetX = 0;
                const targetY = 0;
                const sUfo = instantiate(this.ufoDoubleHit);
                this.ufoHitRoot.addChild(sUfo);
                sUfo.setPosition(targetX, targetY, 0);
              } else {
                const targetX = 0;
                const targetY = 0;
                const dUfo = instantiate(this.ufoDoubleHit);
                this.ufoHitRoot.addChild(dUfo);
                dUfo.setPosition(targetX, targetY, 0);
              }
            }
          }, 1);
        } // Init big bang
...
//# sourceMappingURL=8d8444947818ab17480c74dd9de339799a85b75c.js.map