      ...
      _export("GameRoot", GameRoot = (_dec = ccclass('GameRoot'), _dec2 = property(Node), _dec3 = property(Node), _dec4 = property(Node), _dec5 = property(Prefab), _dec6 = property(Node), _dec7 = property(Prefab), _dec8 = property(Prefab), _dec(_class = (_class2 = class GameRoot extends Component {
        ...
        startCreateEggs() {
          ...
          this.schedule(() => {
            let targetX = 0;
            const targetY = 0;
            let lastPrefabWidth = 0; // 用于跟踪上一个预制体的宽度

            for (let i = 0; i < 2; i++) {
              // 2次循环，每次生成一个预制体
              const randomPrefab = Math.random() < 0.5 ? this.ufoSingleHit : this.ufoDoubleHit;
              const spacingBetweenDefaultPrefabs = 80;
              const spacingBetweenDoubleHitPrefab = 120;
              const ufo = instantiate(randomPrefab);
              this.ufoHitRoot.addChild(ufo); // 根据预制体类型设置间距

              const spacingBetweenPrefabs = randomPrefab === this.ufoDoubleHit ? spacingBetweenDoubleHitPrefab : spacingBetweenDefaultPrefabs; // 调整下一个预制体的位置

              targetX += lastPrefabWidth + spacingBetweenPrefabs;
              lastPrefabWidth = 100; // 记录当前预制体的宽度
...
//# sourceMappingURL=9c09bb6aaca7534196c3b8d6573a3234157d1c0d.js.map