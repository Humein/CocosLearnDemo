      ...
      _export("GameRoot", GameRoot = (_dec = ccclass('GameRoot'), _dec2 = property(Node), _dec3 = property(Node), _dec4 = property(Node), _dec5 = property(Prefab), _dec6 = property(Node), _dec7 = property(Prefab), _dec8 = property(Prefab), _dec(_class = (_class2 = class GameRoot extends Component {
        ...
        startCreateEggs() {
          ...
          this.schedule(() => {
            var targetX = 0;
            var targetY = 0;
            var lastPrefabWidth = 0; // 用于跟踪上一个预制体的宽度

            for (var i = 0; i < 2; i++) {
              // 2次循环，每次生成一个预制体
              var randomPrefab = Math.random() < 0.5 ? this.ufoSingleHit : this.ufoDoubleHit;
              var spacingBetweenDefaultPrefabs = 80;
              var spacingBetweenDoubleHitPrefab = 120;
              var ufo = instantiate(randomPrefab);
              this.ufoHitRoot.addChild(ufo); // 根据预制体类型设置间距

              var spacingBetweenPrefabs = randomPrefab === this.ufoDoubleHit ? spacingBetweenDoubleHitPrefab : spacingBetweenDefaultPrefabs; // 调整下一个预制体的位置
...
//# sourceMappingURL=b24e389207d9843341221f3e67290f1524c127d2.js.map