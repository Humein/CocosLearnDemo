    ...
    execute: function () {
      ...
      _export("item_pic_nodeEvent", item_pic_nodeEvent = (_dec = ccclass('item_pic_nodeEvent'), _dec(_class = (_class2 = class item_pic_nodeEvent extends Component {
        ...
        onTouchAllStart(event) {
          ...
          const transform = this.node.getComponent(UITransform);
          const nodePos = transform.convertToNodeSpaceAR(v3(uiPos.x, uiPos.y, 0)); // 定义边缘触发范围（假设为节点大小的一部分）

          const edgeThreshold = 20; // 可根据需要调整阈值
          // 检查触摸点是否在节点的边缘范围内

          if (nodePos.x > -edgeThreshold && nodePos.x < edgeThreshold && nodePos.y > -edgeThreshold && nodePos.y < edgeThreshold) {
            // 在边缘范围内，触发移动
...
//# sourceMappingURL=e4a17b0b0289adf5d137564cc46f72b4f9bd7331.js.map